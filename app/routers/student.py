...
def create_content(abc: Student):
    cursor.execute("""INSERT INTO students (first_name, last_name) VALUES (%s, %s) RETURNING * """,(abc.first_name,
    abc.last_name))
...
def get_id_content(id:int, response: Response):
    cursor.execute("""SELECT first_name, last_name FROM students WHERE student_id = %s""", (str(id)))
